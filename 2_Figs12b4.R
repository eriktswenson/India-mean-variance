...
# Calculate local correlation and fixed Central India correlation
   for (i in 1:nm) {
       ycor[i,k] = cor(ymean[,k],yvars[i,,k])
      ycor1[i,k] = cor(ymean[,1],yvars[i,,k])
      }
...
# Correlations over 1981-2010 period
      iyrs = 81:110
      ycor30 <- array(0,dim=c(nm,3))
      for (kk in 1:3) for (i in 1:nm) ycor30[i,kk] = cor(ymean[iyrs,kk],yvars[i,iyrs,kk])
      if (!lplotfile) pdf('Fig1a.pdf')
      plot(x1,rev(ycor30[,1]),type='l',col='gray',lwd=5,xlab='period (days)',ylab='',main='',cex.lab=1.5,cex.axis=1.5,cex.main=1.8,xlim=c(2,100),ylim=c(-0.25,0.75),xaxt='n',yaxt='n',log='x')
      ...
      plot(x1,rev(ycor30[,2]),type='l',col='gray',lwd=5,xlab='period (days)',ylab='',main='',cex.lab=1.5,cex.axis=1.5,cex.main=1.8,xlim=c(2,100),ylim=c(-0.25,0.75),xaxt='n',yaxt='n',log='x')
...
for (z in 1:9) {
   ii = 3 + 4*(z-1) + c(1:4)
   yvarave[,z]   = apply(yvars[,,ii],1,mean)
...
for (k in 1:nreg) for (i in 1:n) {
    ycorrun[i,k] = cor(ymean[,k],yruns[i,,k])
   ycorrun1[i,k] = cor(ymean[,1],yruns[i,,k])
   }
...
for (z in 1:9) {
   ii = 3 + 4*(z-1) + c(1:4)
   yrunave[,z]   = apply(yruns[,,ii],1,mean)
